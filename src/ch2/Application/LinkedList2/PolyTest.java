package ch2.Application.LinkedList2;/*
    @Author:***
    @Date:2022/9/22
...
class LPolyNode                        //单链表结点类型
{
    public double coef;                //系数
    public int exp;                    //指数


    LPolyNode(double c, int e)        //重载构造方法
    {
        coef = c;
        exp = e;
    }
...
class PolyClass {
    LinkedList<LPolyNode> poly;//存放多项式顺序表

    public PolyClass() {
        poly = new LinkedList<LPolyNode>();
    }

    public void Add(LPolyNode p) {//末尾添加一个多项式
        poly.add(p);
    }

    public void CreatePoly(double[] a, int[] b, int n) {
        for (int i = 0; i < n; i++) {
            poly.add(new LPolyNode(a[i], b[i]));
        }
    }

    public void Sort() {
        poly.sort(Comparator.comparing(LPolyNode::getExp).reversed());
    }
...
public class PolyTest {
    public static PolyClass Add(PolyClass L1, PolyClass L2) {
        ...
        while (i < L1.poly.size() && j < L2.poly.size()) {
            ...
            {
                L3.Add(new LPolyNode(L1.poly.get(i).coef, L1.poly.get(i).exp));    //新建L3的元素
                i++;
            } else if (L1.poly.get(i).exp < L2.poly.get(j).exp)                        //L2的元素的指数较大
            {
                L3.Add(new LPolyNode(L2.poly.get(j).coef, L2.poly.get(j).exp));    //新建L3的元素
                j++;
            } else                                                                //两元素的指数相等
            {
                c = L1.poly.get(i).coef + L2.poly.get(j).coef;                        //求两指数相等元素的系数和
                if (c != 0)                                                        //系数和c不为0时复制
                    L3.Add(new LPolyNode(c, L1.poly.get(i).exp));                    //新建L3的元素
                i++;
                j++;
            }
        }
