...
public class SecondOrder<E> {
    LinkNode<E> head;//存放头结点

    public SecondOrder() {//构造方法
    ...
    public E getElement(int i) {//查找序号为i(0≤i≤n-1)的元素
        int len = size();
        if (i < 0 || i > len - 1) {
            throw new IllegalArgumentException("设置长度：i不在有效范围内");
        }
    ...
    public void deleteAny(int i) {//在线性表中删除序号i位置的元素
        if (i < 0 || i > size() - 1) {//参数错误抛出异常
            throw new IllegalArgumentException("设置位置：i不在有效范围内");
        }
...
class Test1 {
    public static void main(String[] args) {
        SecondOrder<Integer> list = new SecondOrder<>();
